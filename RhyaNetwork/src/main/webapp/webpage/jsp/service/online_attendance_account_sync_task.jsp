...
<html>
	<head>
		<title>RHYA.Network Online Attendance Account Sync</title>
		<meta charset="utf-8">
	...
	if (cont != null) {
		...
		stat.setInt(1, JspPageInfo.PageID_Online_Attendance_Account_Sync_Email_Task);
		...
		if (!JspPageInfo.JspPageStateManager(state)) {
			...
			String auth_uuid = request.getParameter("a_uuid");
			String teacher_uuid = request.getParameter("t_uuid");
			// Null 확인
			if (user_uuid == null ||
				auth_uuid == null ||
				teacher_uuid == null) {
			...
			try {
				...
				teacher_uuid = urlFilter.GetFilter(teacher_uuid);
				auth_uuid = urlFilter.GetFilter(auth_uuid);
				user_uuid = RhyaAES.AES_Decode(user_uuid);
				teacher_uuid = RhyaAES.AES_Decode(teacher_uuid);
				auth_uuid = RhyaAES.AES_Decode(auth_uuid);
				// 로그 출력
				rl.Log(RhyaLogger.Type.Info, rl.CreateLogTextv1(clientIP, new String[] { "u_uuid", "t_uuid", "a_uuid" },
																		  new String[] { user_uuid, teacher_uuid, auth_uuid }));
				// 데이터베이스 접속
				stat.close();
				stat = cont.GetConnection().prepareStatement("SELECT * FROM online_attendance_account_sync WHERE teacher_uuid = ? AND uuid = ? AND account_sync_code = ?;");
				stat.setString(1, teacher_uuid);
				stat.setString(2, user_uuid);
				stat.setString(3, auth_uuid);
				// 쿼리 실행
				rs = stat.executeQuery();
				// 쿼리 실행 결과
				if (rs.next()) {
					// 결과 설정
					if (rs.getString("uuid").equals(user_uuid)) {
						stat.close();
						stat = cont.GetConnection().prepareStatement("UPDATE online_attendance_account_sync SET isAccess = 1 WHERE teacher_uuid = ? AND uuid = ? AND account_sync_code = ?;");
						stat.setString(1, teacher_uuid);
						stat.setString(2, user_uuid);
						stat.setString(3, auth_uuid);
						stat.executeUpdate();
						...
						// 로그 출력
						rl.Log(RhyaLogger.Type.Warning, rl.CreateLogTextv5(clientIP, "온라인 출석부 계정 연동 성공!"));
					}else {
						// 로그 출력
						rl.Log(RhyaLogger.Type.Warning, rl.CreateLogTextv5(clientIP, "온라인 출석부 계정 연동 실패!"));
						// 연결 종료
						rs.close();
						stat.close();
						cont.Close();
