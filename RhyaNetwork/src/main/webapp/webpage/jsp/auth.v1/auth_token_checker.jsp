...
try {
	...
	if (!authParm.SERVICE.contains(auth_token_name)) {
		// 로그 출력
		rl.Log(RhyaLogger.Type.Info, rl.CreateLogTextv5(clientIP, "Auth Token Check Error : 지원하는 서비스가 아님"));
		...
		if (cont != null) {
			...
			if (!JspPageInfo.JspPageStateManager(state)) {
				// 로그 출력
				rl.Log(RhyaLogger.Type.Warning, rl.CreateLogTextv5(clientIP, "Auth Token Check Error : Page 접근 거부"));
				...
				if (auth_token_uuid != null && auth_token_name != null) {
					...
					if (rs.next()) {
						...
						if (result[0].equals(AuthTokenChecker.AUTH_RESULT_SUCCESS)) {
							// 로그 출력
							rl.Log(RhyaLogger.Type.Info, rl.CreateLogTextv8(clientIP, "Auth Token Check Success : Token is enabled /", auth_token_uuid));
							...
							// 로그 출력
							rl.Log(RhyaLogger.Type.Info, rl.CreateLogTextv5(clientIP, "Auth Token Check Error : Not defined token"));
					...
					// 로그 출력
					rl.Log(RhyaLogger.Type.Info, rl.CreateLogTextv5(clientIP, "Auth Token Check Error : 파라미터 == Null"));
